...
def admin():
    if str(current_user).split()[2] == 'ADMIN' or str(current_user).split()[2] == 'MODER':
        return render_template("admin.html", title='DinEsAidDe')
    else:
        return redirect("/")
    ...
    if str(current_user).split()[2] == 'ADMIN' or str(current_user).split()[2] == 'MODER':
        if form.validate_on_submit() and form.email.data != str(current_user).split()[1]:
            ...
            if not stop:
                ...
                con.close()
                ###
        return render_template('perms.html', form=form, title='DinEsAidDe')
    else:
        return redirect("/")
...
def orders():
    if str(current_user).split()[2] == 'ADMIN' or str(current_user).split()[2] == 'MODER':
        ...
        con.commit()
        ###
        return render_template('orders.html', hm=s, title='DinEsAidDe')
    else:
        return redirect("/")
...
def products():
    if str(current_user).split()[2] == 'ADMIN' or str(current_user).split()[2] == 'MODER':
        return render_template("products.html", title='DinEsAidDe')
    else:
        return redirect("/")
...
def products_add():
    ...
    if str(current_user).split()[2] == 'ADMIN' or str(current_user).split()[2] == 'MODER':
        if form.validate_on_submit():
            ...
            con.close()
            ###
        return render_template("add.html", form=form, title='DinEsAidDe')
    else:
        return redirect("/")
...
def products_remove():
    ...
    if str(current_user).split()[2] == 'ADMIN' or str(current_user).split()[2] == 'MODER':
        if form.validate_on_submit():
            ...
            con.close()
            ###
        return render_template("remove.html", form=form, title='DinEsAidDe')
    else:
        return redirect("/")
...
def index():
    ...
    con.close()
    ###
    return render_template("index.html", len_hm=len(img), names=name, ds=description, prices=price, hm=img, id=pid, title='DinEsAidDe')
...
def buy(item):
    ...
    if form.validate_on_submit() and form.email.data == str(current_user).split()[1]:
        ...
        con.close()
        ###
        return render_template("buy.html", title='DinEsAidDe', message='Заказ успешно оформлен, для того, чтобы его оплатить, переведите указанную сумму на карту по номеру +7 928 815 50 01', form=form, painting=painting, price=price, p_img=p_img)
    else:
        return render_template("buy.html", title='DinEsAidDe', form=form, painting=painting, price=price, p_img=p_img)
